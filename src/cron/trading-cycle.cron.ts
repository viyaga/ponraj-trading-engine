...
const tradingCycleCronJob = (): void => {
    ...
    cron.schedule(env.cronSchedule ?? "*/1 9-15 * * 1-5", async () => {
        ...
        // ── 3. Hourly Candle Boundary Recalculation (:15 of every hour) ──────
        // 1H candles close at 10:15, 11:15, 12:15, 13:15, 14:15, 15:15 IST
        if (istMinute === 15) {
            tradingCronLogger.info("[TradingCron] 🔔 1-Hour candle boundary reached (:15 IST) — refreshing UT Bot trigger thresholds...");
